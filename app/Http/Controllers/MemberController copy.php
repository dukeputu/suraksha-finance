...
// User App All Cntoler Start *************************
    ...
    public function registerUserApp(Request $request)
    {
        ...
        DB::table('app_users')->insert([
            ...
            'introducer_phone'  => $introducer->phone_number ?? '0102030405',
            'introducer_name'   => $introducer->app_u_name ?? 'Company',
            'user_email'        => $request->user_email,

            'introducer_wallet' => $introducer->user_wallet ?? '0',
...
// ************************************************************
    ...
    public function addBalanceTrafer(Request $request, $id)
    {
        ...
        DB::beginTransaction();
        try {
            // Step 1: Update Balance Request
            DB::table('user_balance_request')->where('id', $id)->update([
                'status'     => 1,
                'updated_at' => now(),
            ]);

            // Step 2: Update User Wallet + Plan
            ...
            $this->distributeCommission($user->id, $balanceRequest->selected_plan_id);
            ...
            DB::commit();
            return back()->with('success', 'Balance transferred & commission distributed successfully.');
    ...
    private function distributeCommission($userId, $planId)
    {
        $user = DB::table('app_users')->where('id', $userId)->first();
        if (! $user) {
            return false;
        ...
        while ($level <= 40) {
            ...
            $introducer = DB::table('app_users')->where('id', $introducerId)->first();
            if (! $introducer) {
                break;
            }

            // Downline rank (the one who purchased plan)
            $downlineRank = $user->select_plan_id;

            // Upline rank (the introducer)
            $uplineRank = $introducer->select_plan_id;

            $commission = 0;

            // ------------------- Commission Logic -------------------
            if ($uplineRank > $downlineRank) {
                // Upline rank বড় -> কমিশন আসবে Downline এর payout table থেকে
                $commission = DB::table('plan_master')
                    ->where('select_plan_id', $planId)
                    ->where('plan_level', 'L' . $level)
                    ->value('plan_payout') ?? 0;

            } elseif ($uplineRank < $downlineRank) {
                // Upline rank ছোট -> কমিশন আসবে নিজের payout table থেকে
                $commission = DB::table('plan_master')
                    ->where('select_plan_id', $uplineRank)
                    ->where('plan_level', 'L' . $level)
                    ->value('plan_payout') ?? 0;

            } else {
                // Same rank -> কমিশন আসবে নিজের payout table থেকে
                $commission = DB::table('plan_master')
                    ->where('select_plan_id', $uplineRank)
                    ->where('plan_level', 'L' . $level)
                    ->value('plan_payout') ?? 0;
            ...
            if ($commission > 0) {
                ...
                // Update introducer wallet
                DB::table('app_users')
                    ->where('id', $introducer->id)
                    ->increment('user_wallet', $commission);
            }
